...
const RideCard = ({ ride: {
    destination_address, destination_latitude, destination_longitude, user_email, created_at, origin_address, ride_time, driver, payment_status
} }: { ride: Ride }) => {

    // console.log("API ey is", `{https://maps.geoapify.com/v1/staticmap?style=osm-bright-smooth&width=600&height=400&center=lonlat:${destination_longitude},${destination_latitude}&zoom=14&apiKey=${process.env.EXPO_PUBLIC_GEOAPIFY_API_KEY}`)
    return (
            ...
            <View className="flex flex-col justify-center items-center p-3">
                ...
                <View className="flex flex-col w-full mt-5 bg-general-500 rounded-lg p-3 items-start justify-center">
                    <View className="flex flex-row items-center w-full justify-between mb-5">
                        <Text className="text-md font-JakartaMedium text-gray-500">Date and Time</Text>
                    ...
                    {/* Payment Container */}
                    <View className="flex flex-row items-center w-full justify-between mb-5">
                        <Text className="text-md font-JakartaMedium text-gray-500">Last Payment</Text>
                        <Text className={`text-md capitalize font-JakartaMedium text-gray-500 ${payment_status === 'paid' ? 'text-green-400' : 'text-red-500 font-bold'}`}>{payment_status}</Text>
                    </View>

                    {/* Car Container */}
                    <View className="flex flex-row items-center w-full justify-between mb-5">
                        <Text className="text-md font-JakartaMedium text-gray-500">No. of Seats</Text>

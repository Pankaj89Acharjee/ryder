...
const CustomButton = ({ onPress, title, bgVariant = "primary", textVariant = "default", IconLeft, IconRight, className, ...props }: ButtonProps) => {
    return (
        <TouchableOpacity
            className={`w-full rounded-full p-3 justify-center items-center flex flex-row shadow-md shadow-neutral-400/70 ${getBgVariantStyle(bgVariant)} ${className}`} {...props}
            onPress={onPress}>
            {IconLeft && <IconLeft />}
